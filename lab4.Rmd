...
heat_map_plot <- function(col, year){
  filtered_df <- subset(salary_df, year == {{year}})
  ggplot(filtered_df, aes(x = factor({{col}}, 
                                   levels = unique({{col}})), 
                        y = salary_exp_post_grad)) + 
  geom_bin2d() +
  scale_x_discrete() + #x axis name should be col_name
  scale_y_discrete(limits = rev(levels(salary_df$salary_exp_post_grad))) +
  ggtitle("Year = 2019") #change to {{year}}
}

#heat_map_plot(mds_self_rated_enjoy, 2020)
...
# You plot goes here.
par(mfrow = c(1, 2))
plot1 <- heat_map_plot(mds_self_rated_enjoy, 2019)
plot2 <- heat_map_plot(mds_self_rated_enjoy, 2020)
